...
var tallChartGenerator = function (image_dimension) {
	...
	function generateHypnogram(datafile, imageSize, i) {
		...
		var margin = {
			...
			width = (160 - margin.left - margin.right), // 240-10 => 480 min = 230px => 1min = .47px => 18 min = 18 x .47 =  8.47 px
		...
		d3.csv(datafile, function (error, data) {
			...
			function customYAxis(g) {
				...
				g.attr("transform", "translate(0,7)")  // tick position
			...
			function customXAxis(g) {
				...
				g.selectAll(".tick text").attr("x", -3).attr("dy", ".71em"); //tick mark to text
